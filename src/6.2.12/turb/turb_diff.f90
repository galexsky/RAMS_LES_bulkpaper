...
do j = ja,jz
   do i = ia,iz
      do k = 2,m1-1
         ...
         if(k==2 .and. nstbot .eq. 1) then
          !Saleeby: adding surface flux tendency to the end rather than
          !running this thru diffusion code. This should help better link
          !fluxes from land surface model to atmosphere for THP and RTP.
          sct(k,i,j) = sct(k,i,j) + vt3df(k,i,j) * dtlti &
            + sfcflx(i,j) * dzt(2) * dn03i(k,i,j) / rtgt(i,j)
         else
          sct(k,i,j) = sct(k,i,j) + vt3df(k,i,j) * dtlti
         endif
      enddo
