...
implicit none
  ...
  do igrid = 1, ngrids
    ...
    enddo
  enddo
!SWF Edit 7/29/20: We need to deal with the special case of varfile and/or surface file write.
!In this case, we will have nmachs=1 BUT we don't want the chunksize to be equal to the varfile size.
!As a reminder, chunks are laid out here as:       f_chunks(1:3) = (/ file_xchunk(igrid), file_ychunk(igrid), mmzp(igrid) /)
!there is a different debate that should take place about how these should be chunked along Z (currently they aren't).
  print*, "SWF: nmachs is: ", nmachs
  if (nmachs == 1) then
    do igrid = 1, ngrids
      !let's try a max chunk size for X and Y of 50 points. This might be too small or too large, but it's a starting point.
      print*, "Setting file_xchunk to: ", min(100, nnxp(igrid))
      file_xchunk(igrid) = min(100, nnxp(igrid))
      file_ychunk(igrid) = min(100, nnyp(igrid))
      print*, "file_xchunk now ", file_xchunk(igrid)
    enddo
