...
if (level <= 3) then
   CALL adj1 (m1,m2,m3,basic_g(ngrid)%rtp(1,1,1),micro_g(ngrid),ngrid)
elseif (level == 4) then
   CALL adj1_bin (m1,m2,m3,basic_g(ngrid)%rtp(1,1,1),micro_g(ngrid))
endif
...
do j = 1,m3
   ...
   do k = 1,m1
     if (rtemp(k) > rtp(k,i,j)) then
       frac = rtp(k,i,j) / max(1.e-12,rtemp(k))
       do lcat = 1,ncat
...
do k = ka,kb
   c1(k) = c1(k) * r1(k) / max(1.e-12,r3(k))
   if (c1(k) < 0.) c1(k) = 0.
enddo
