!##############################################################################
Subroutine dust_sources (m1,m2,m3,ia,iz,ja,jz,rtgm,glat,glon         &
                        ,u,v,dn0,md1np,md2np,md1mp,md2mp             &
                        ,dustfrac,swater,parea,lclass                &
                        ,vegrough,stext                              &
                        )
...
real, dimension(m2,m3) :: u10,v10,rtgm,num_dust_s,num_dust_l &
                         ,glat,glon,dustfrac
...
real, dimension(m2,m3,npatch) :: lclass,vegrough
 ...
 CALL dustflux (u10,v10,swater,npatch,m1,m2,m3,ia,iz,ja,jz     &
       ,zm,rtgm,dtlt,num_dust_s,num_dust_l,ncls                &
       ,z0limit,veglimit,glat,glon,dustfrac,parea,nzg,lclass   &
       ,stext,vegrough                                         &
       )
...
!##############################################################################
Subroutine salt_sources (m1,m2,m3,ia,iz,ja,jz,rtgm,u,v,dn0       &
...
Subroutine dustflux (u10,v10,swater,npatch,m1,m2,m3,ia,iz,ja,jz     &
         ,zm,rtgm,dtlt,num_dust_s,num_dust_l,ncls                   &
         ,z0limit,veglimit,glat,glon,dustfrac,xparea,nzg,xlclass    &
         ,xstext,xvegrough                                          &
         )
...
use micphys, only:idustloft,iscm
use aero_include
...
real, dimension(m1) :: zm
real, dimension(m2,m3) :: num_dust_s,num_dust_l,wind,u10,v10,rtgm &
                         ,glat,glon,dustfrac
...
if(ISCM==1)then
 ...
 netrough = 3.00e-5
   ...
   if(vegstop==0) then
    ...
    elseif(idustloft==1 .or. idustloft==2 .or. idustloft==22) then
      !If using Ginoux et al.(2001) or NRL database
