...
if(ncall == 0) then
   ncall = 1
   !Allocate hydrometeor sedimentation table arrays
   do ngr = 1,ngrids
    allocate (pcp_tab(ngr)%pcpfillc(mmzp(ngr),maxkfall,nembfall &
                                   ,nhcat,ndensrtgt,nband))
    allocate (pcp_tab(ngr)%pcpfillr(mmzp(ngr),maxkfall,nembfall &
                                   ,nhcat,ndensrtgt,nband))
    allocate (pcp_tab(ngr)%sfcpcp(maxkfall,nembfall             &
                                   ,nhcat,ndensrtgt,nband))
    allocate (pcp_tab(ngr)%allpcp(mmzp(ngr),nembfall            &
                                   ,nhcat,ndensrtgt,nband))
   enddo
...
do j = ja,jz
   do i = ia,iz
      ...
      CALL range_check (mzp,k1,k2,k3,i,j,frqstate(ngr),ngr,dtlt,time &
                      ,ioutput,micro_g(ngr))
      ...
      !if(i+mi0(ngr)==2 .and. j+mj0(ngr)==15) then
      ! do k=2,mzp,1
      !  print'(5i4,1f10.1,2X1f20.10)',k,i,ia,j,ja,time &
      !   ,micro_g(ngr)%cifnp(k,i-1,j)
      ! enddo
      !endif
   enddo
enddo
