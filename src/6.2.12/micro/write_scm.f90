...
!##############################################################################
Subroutine readwrite_scm (m1,i,j,wf,fln)
...
print*,'Writing micro output for SCM'
...
call iofil('soil_water.txt',leaf_g(ng)%soil_water(nzg,i,j,2),1,wf,fln)
...
call iofil('topm.txt',grid_g(ng)%topm(i,j),1,wf,fln)
...
if(idust > 0) then
 ...
 call iofil('dustfrac.txt',micro_g(ng)%dustfrac(i,j),1,wf,fln)
endif
