...
use micphys,   only:level,icheckmic,iscm
use mem_grid
...
!##############################################################################
!SINGLE COLUMN SECTION NEEDED FOR COMPARISON
!##############################################################################
if(iscm==1.and.time==1860.) CALL readwrite_scm (mzp,15,21,2,"zz.in/")
...
if(iscm==1.and.time==1860.) CALL readwrite_scm (mzp,15,21,3,"zz.out/")
!##############################################################################
!##############################################################################
...
if(ngrid==100 .and. string.eq.'VPSETS') then
do kp=1,40
do ip=1,80
do jp=1,80 
 if(micro_g(ngrid)%cccnp(kp,ip,jp)>631.0) &
 print'(a,3i5,2e17.9)','Steve',kp,ip,jp,micro_g(ngrid)%cccnp(kp,ip,jp)
enddo
enddo
enddo
endif
...
!   print 101, my_rams_num, kp, ip, jp, k, i, j, 'cifnp', string, time &
!     ,micro_g(ngrid)%cifnp(k,i,j)
